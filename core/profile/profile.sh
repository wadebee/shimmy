# Profile selection and installed-layout helpers.
...
shimmy_upstream_checkout_invalid_reason() {
  ...
  for required_dir in commands core tools; do
  ...
  for required_file in \
    ...
    core/common/common.sh \
    core/profile/profile.sh \
    core/catalog/catalog.sh
  do
  ...
  if [ -n "$shim_name" ] && [ ! -f "$checkout_dir/tools/$shim_name/tool.conf" ]; then
