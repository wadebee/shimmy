#!/bin/sh
# Refresh the OpenShift CLI 4.20 local image when requested by shimmy update.
set -eu
...
case "${1:-}" in
  ...
  build)
    ...
    shimmy_local_image_stale_cleanup "localhost/shimmy-oc-4_20" "$SCRIPT_DIR/container"
    ;;
  *)
