#!/bin/sh
set -eu

SCRIPT_DIR=$(cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(cd -- "$SCRIPT_DIR/../../../.." && pwd)
SHIMMY_RUNTIME_DIR=$ROOT_DIR/lib/runtime
SHIMMY_IMAGE_CONFIG_FILE=$SCRIPT_DIR/image.conf
SHIMMY_IMAGE_HELPER_FILE=$SHIMMY_RUNTIME_DIR/image.sh

SHIMMY_YQ_IMAGE_PULL=${SHIMMY_YQ_IMAGE_PULL:-}
SHIMMY_YQ_PULL_ARG=

if [ ! -f "$SHIMMY_IMAGE_HELPER_FILE" ]; then
  printf 'ERROR: missing shim helper: %s\n' "$SHIMMY_IMAGE_HELPER_FILE" >&2
  exit 1
fi

# shellcheck source=lib/runtime/image.sh
. "$SHIMMY_IMAGE_HELPER_FILE"

SHIMMY_YQ_IMAGE=${SHIMMY_YQ_IMAGE:-$(shimmy_image_external_default_read "$SHIMMY_IMAGE_CONFIG_FILE")}

shimmy_podman_preflight_or_preview_require "the yq shim" "$@"

if [ "$SHIMMY_YQ_IMAGE_PULL" = "always" ]; then
  SHIMMY_YQ_PULL_ARG=--pull=always
fi

# Map the rootless engine user to the upstream image's non-root yq user.
shimmy_podman_run_or_preview "$SHIMMY_PODMAN_BIN" run --rm -i \
  --platform "$SHIMMY_PODMAN_PLATFORM" \
  ${SHIMMY_YQ_PULL_ARG:+"$SHIMMY_YQ_PULL_ARG"} \
  --network none \
  --cap-drop all \
  --security-opt no-new-privileges \
  --userns keep-id:uid=1000,gid=1000 \
  --user 1000:1000 \
  -v "$PWD:/work" \
  -w /work \
  "$SHIMMY_YQ_IMAGE" \
  "$@"
