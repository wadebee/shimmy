#!/bin/sh
# Refresh the jv 6.0 local image during shim synchronization.
set -eu
...
# shellcheck source=tools/jv/versions/6.0/smoke.conf
...
case "${1:-}" in
  pull)
    ;;
  build)
    [ -z "${SHIMMY_JV_IMAGE:-}" ] || exit 0
    SHIMMY_JV_IMAGE_BUILD=always "$SCRIPT_DIR/run.sh" "$smoke_arg" >/dev/null </dev/null
