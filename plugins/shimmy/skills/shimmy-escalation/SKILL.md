...
3. Discover activated shims:
   ...
   - If the status command is unavailable, inspect `$SHIMMY_INSTALL_DIR/profiles/default/install-manifest.txt`, `$SHIMMY_INSTALL_DIR/profiles/upstream/install-manifest.txt`, then `$SHIMMY_INSTALL_DIR/install-manifest.txt`.
