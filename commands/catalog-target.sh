#!/bin/sh
# Private candidate catalog command. This file is intentionally absent from the
# installed dispatcher until the control-surface cutover.
set -eu
...
usage() {
  cat <<'EOF'
Private target default-catalog candidate.
...
[ -n "$target_config_root" ] || fail 'SHIMMY_TARGET_CONFIG_ROOT is required for private target catalog routing'
