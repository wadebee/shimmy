#!/bin/sh
# Uninstalled private target AI-skill lifecycle candidate.
set -eu
...
usage() {
  cat <<'EOF'
Private target profile AI-skill lifecycle candidate.

Usage:
  ai-skill-target.sh list [--format human|manifest]
  ai-skill-target.sh repair
...
[ -n "$shimmy_target_config_root" ] || fail 'SHIMMY_TARGET_CONFIG_ROOT is required for private target AI-skill routing'
...
case "$shimmy_target_action" in
  list)
    ...
    shimmy_target_ai_skill_list_render "$shimmy_target_format" || fail 'unable to inspect target AI-skill bundles and links'
    ;;
  repair)
    ...
    ;;
  *) fail "unknown target AI-skill action: $shimmy_target_action" ;;
esac
