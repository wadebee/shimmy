...
only tracked `tools/`, validates them, and creates or reuses a content-addressed
generation containing `generation.conf` and `tools/`. Generation metadata
records parser identity, provenance, and the tools-only fingerprint outside the
fingerprint scope. A reused generation retains its original provenance commit;
publishing equivalent current content does not rewrite the registry or advance
current/previous. `rollback` selects the retained previous valid generation.
Neither command updates profile source commits or catalog pins, and neither
deletes retained generations. Use explicit `profile sync` to adopt newer
control-source or catalog authority.
