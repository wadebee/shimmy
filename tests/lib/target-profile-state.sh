...
test_lib_target_profile_state_round_trip() {
  ...
  shimmy_target_profile_manifest_read "$target_root/profile/install-manifest.txt" || fail_test 'valid target profile manifest rejected'
  if shimmy_profile_manifest_validate "$target_root/profile/install-manifest.txt" team-one >/dev/null 2>&1; then
    fail_test 'current version-1 reader accepted a target version-2 manifest'
  fi
  if shimmy_target_profile_manifest_read "$SHIMMY_TEST_PROFILE_FIXTURES_ROOT/default/install-manifest.txt"; then
    fail_test 'target version-2 reader accepted a current version-1 manifest'
  fi
  shimmy_target_profile_manifest_read "$target_root/profile/install-manifest.txt" || fail_test 'target manifest could not be reread after version-boundary checks'
