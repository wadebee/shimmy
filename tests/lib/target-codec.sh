...
test_lib_target_codec_tokens() {
  for valid_name in default a alpha-1 team-one-two; do
    shimmy_name_component_validate "$valid_name" || fail_test "valid target name rejected: $valid_name"
  done
  for invalid_name in '' -alpha alpha- alpha--beta alpha.beta alpha_beta Alpha 'alpha beta' 'alpha/beta'; do
    if shimmy_name_component_validate "$invalid_name"; then
      fail_test "unsafe target name accepted: $invalid_name"
    fi
  done
  ...
  pass 'target name and commit algorithms enforce fixed grammars'
}

test_lib_target_codec_hash_vectors() {
  setup_scenario
  printf 'Shimmy target content\n' > "$SCENARIO_DIR/content"
  assert_equals "$(shimmy_sha256_fingerprint_file_render "$SCENARIO_DIR/content")" \
    sha256:340e10a2f3da257f9fb66bbc716fb3e415ba0306608942a1871c641e502f71ca
  vector_fingerprint=sha256:584a0bb2995f58fa4150c3662f3a8c5cfe89dc9f4b5da6790bd9641d2c7e3bc9
  assert_equals "$(shimmy_target_catalog_generation_render "$vector_fingerprint")" \
  ...
  assert_equals "$(shimmy_target_catalog_content_fingerprint_render "$SCENARIO_DIR/catalog")" \
...
test_lib_target_codec_manifest_vectors() {
