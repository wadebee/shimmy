...
test_commands_target_lifecycle_end_to_end() {
  ...
  for target_lifecycle_asset_profile in default team-one; do
    ...
    target_lifecycle_asset_commands=$(find "$target_lifecycle_asset_root/commands" \
      -mindepth 1 -maxdepth 1 -type f -exec basename {} \; | LC_ALL=C sort)
    assert_equals "$target_lifecycle_asset_commands" 'admin.sh
ai-skill.sh
catalog.sh
help.sh
profile.sh
shim.sh'
    for target_lifecycle_asset_command in admin.sh ai-skill.sh \
  ...
  target_lifecycle_failed_output=$(env HOME="$TARGET_LIFECYCLE_HOME" \
    ...
    "$TARGET_LIFECYCLE_CHECKOUT/commands/bootstrap.sh" --no-startup 2>&1)
