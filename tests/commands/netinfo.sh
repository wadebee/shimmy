...
test_commands_netinfo_cidr_rendering() {
  output=$(run_in_repo ./commands/netinfo.sh --format manifest --host-ip 198.51.100.255 --host-prefix 25 --target 198.51.100.1)
...
test_commands_netinfo_explicit_lan_precedence() {
  output=$(run_in_repo ./commands/netinfo.sh --format manifest --host-ip 203.0.113.99 --host-prefix 24 --host-lan 203.0.113.64/26)
...
test_commands_netinfo_help() {
  output=$(run_in_repo ./commands/netinfo.sh --help)
...
test_commands_netinfo_invalid_inputs() {
  set +e
  invalid_ip_output=$(run_in_repo ./commands/netinfo.sh --host-ip 198.51.100.256 2>&1)
  invalid_ip_status=$?
  invalid_prefix_output=$(run_in_repo ./commands/netinfo.sh --host-prefix 33 2>&1)
  invalid_prefix_status=$?
  invalid_lan_output=$(run_in_repo ./commands/netinfo.sh --host-lan 203.0.113.1 2>&1)
  invalid_lan_status=$?
  set -e
