...
test_commands_install_run() {
  ...
  for claimed_path in shell-init.sh registries.conf install-manifest.txt bin bin/shimmy bin/jq commands config implementations lib plugins tests tools; do
