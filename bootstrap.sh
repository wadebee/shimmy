...
shimmy__bootstrap_run() {
  ...
  for shimmy__target_bootstrap_candidate in "$shimmy__target_bootstrap_script_root" \
    ...
    [ -x "$shimmy__target_bootstrap_candidate/commands/bootstrap.sh" ] || continue
    [ -f "$shimmy__target_bootstrap_candidate/lib/install/lifecycle.sh" ] || continue
    shimmy__target_bootstrap_root=$shimmy__target_bootstrap_candidate
    break
  done
  [ -x "$shimmy__target_bootstrap_root/commands/bootstrap.sh" ] || {
    printf '%s\n' 'ERROR: Shimmy bootstrap command is unavailable' >&2
    return 1
  }
  "$shimmy__target_bootstrap_root/commands/bootstrap.sh" "$@" || return $?
