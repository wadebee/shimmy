...
- Preserve `AWS_IMAGE`, `SHIMMY_AWS_IMAGE_PULL`, the `$PWD` mount, the optional `~/.aws` read-only mount, and `AWS_*` forwarding unless the task changes those behaviors on purpose.
