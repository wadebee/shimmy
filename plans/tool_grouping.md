...
- **Kind**: the stable user-facing command, such as `jq`, `rg`, `oc`, or
  `terraform`.
- **Version**: the concrete implementation selected by a kind, such as
  `oc_4_20`.

Grouping should sit above kinds. It should select multiple existing kinds for
...
- Keep the existing kind/version model intact.
...
- Fits the earlier reserved meaning: organizational grouping above tool kinds.
...
- Slightly informal.
- Needs clear documentation that a family selects tool kinds, not concrete
  versions.
...
- A tool kind may belong to many families.
- `shimmy install --family <name>` expands to the current default version of
  every kind in that family.
- Multiple requested families form a union and deduplicate kinds.
...
This model lets each tool kind declare several supported family labels.
...
- `--family` may be combined with `--shim`; the final kind set is the union.
- Unknown families fail and print available families.
- Empty families fail in tests.
- Installed duplicate kind/version entries are deduplicated through the
  existing kind resolver.
- Family install uses each kind's default version.
...
The profile manifest should continue to record installed `kind=` and
`kind_version=` entries as the source of truth. A family install is just a
...
shimmy_available_family_kind=networking|nmap
```
...
- Tool metadata: add family labels to `tools/<kind>/tool.conf` or an adjacent
  catalog-owned metadata file.
- Catalog helpers: list supported families and resolve family names to kinds.
- Install request parsing: accept repeatable `--family <name>` and merge with
  `--shim` requests.
- Test selection: accept `--family <name>` and expand to kind tests.
...
- **Default family confusion**: The existing default install set should remain
  separate from families. `jq` and `rg` being default kinds does not mean there
  is a special `default` family.
- **Version expectations**: A family should install the default version for each
  kind. Users who need a non-default version should still request
  `--shim kind@version`.
...
- Should family metadata live in each `tool.conf`, or should a central
  family registry map families to kinds?
...
- Existing `--shim <kind>[@<version>]` behavior remains unchanged.
