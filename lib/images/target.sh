#!/bin/sh
# Private target catalog image verification. Public image routing continues to
# use commands/images.sh until the control-surface cutover.
...
shimmy_target_images_verify_run() {
  ...
  for shimmy_target_images_version_name in $shimmy_target_images_selected_versions; do
    shimmy_target_images_tool_name=$(shimmy_tool_version_tool "$shimmy_target_images_version_name") || {
      shimmy_target_images_error_set "unknown selected concrete version: $shimmy_target_images_version_name"
      return 1
    }
