...
shimmy_target_profile_control_assets_copy() {
  ...
  [ -d "$shimmy_target_profile_control_source/commands" ] &&
    [ ! -L "$shimmy_target_profile_control_source/commands" ] || return 1
  [ -d "$shimmy_target_profile_control_source/lib" ] &&
    [ ! -L "$shimmy_target_profile_control_source/lib" ] || return 1
  mkdir -p "$shimmy_target_profile_control_destination/commands" || return 1
  for shimmy_target_profile_control_command in admin ai-skill catalog help profile shim; do
    [ -f "$shimmy_target_profile_control_source/commands/$shimmy_target_profile_control_command.sh" ] &&
      [ ! -L "$shimmy_target_profile_control_source/commands/$shimmy_target_profile_control_command.sh" ] || return 1
    cp "$shimmy_target_profile_control_source/commands/$shimmy_target_profile_control_command.sh" \
      "$shimmy_target_profile_control_destination/commands/$shimmy_target_profile_control_command.sh" || return 1
  done
  cp -R "$shimmy_target_profile_control_source/lib" \
    "$shimmy_target_profile_control_destination/lib" || return 1
}
